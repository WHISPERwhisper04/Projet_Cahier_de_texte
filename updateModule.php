...
include_once('Module.php');      // Inclusion de la classe Module
...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    if (isset($_GET['id_module'])) {
        ...
        $module = Module::selectModuleById('module', $connection->conn, $id);
        if ($module) {
            $nomModuleValue = $module['nom_module'];
            $descriptionValue = $module['description'];
        } else {
            $errorMessage = "Module not found!";
        }
    } else {
        $errorMessage = "Module ID is missing!";
    }
}
...
// Si le formulaire est soumis (POST)
    ...
    $nomModuleValue = $_POST["nom_module"];
    $descriptionValue = $_POST["description"];

    // Vérification des champs obligatoires
    if (empty($nomModuleValue) || empty($descriptionValue)) {
        $errorMessage = "All fields must be filled out!";
    } else {
        // Créer une nouvelle instance de Module
        $module = new Module($nomModuleValue, $descriptionValue);
        // Appeler la méthode pour mettre à jour le module
        Module::updateModule($module, 'module', $connection->conn, $_GET['id_module']);
...
<body>
    <div class="container my-5">
        ...
        <form method="post">
            <div class="row mb-3">
                <label class="col-form-label col-sm-2" for="nom_module">Module Name:</label>
                <div class="col-sm-6">
                    <input value="<?php echo $nomModuleValue ?>" class="form-control" type="text" id="nom_module" name="nom_module">
                </div>
            </div>
            <div class="row mb-3">
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-3 d-grid">
                    <a class="btn btn-outline-primary" href="readModule.php">Cancel</a>
                </div>
            </div>
        </form>
